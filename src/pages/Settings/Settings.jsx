...
const AiContactForm = ({
  ...
  const typeOptions = useMemo(() => {
    const types =
      elements.types || elements.typeOptions || elements.contactTypes;
  ...
  const relationshipOptions = useMemo(() => {
    ...
    const rel =
      ...
      elements.relationshipOptions ||
      elements.relationship;

    return Array.isArray(rel) && rel.length
      ? rel
      : [
          "No Relationship",
          "Friend",
  ...
  const expertiseOptions = useMemo(() => {
    const exp =
      elements?.elements?.experties ||
      elements.expertiseOptions ||
      elements.expertise;
    return Array.isArray(exp) && exp.length
      ? exp
      : [
          "No Expertise",
          "Wellness Coach",
  ...
  const wantToHearOptions = useMemo(() => {
    const wth =
      elements?.elements?.whatYouWantToHear ||
      elements.wantToHearOptions ||
      elements.whatDoYouWantToHear;
    return Array.isArray(wth) && wth.length
      ? wth
      : [
          "Random Message",
          "Greeting",
  ...
  const characteristicOptions = useMemo(() => {
    const chars =
      elements?.elements?.characteristics || elements.charactersticsOptions;
    return Array.isArray(chars) && chars.length
      ? chars
      : [
          "Adaptable",
          "Adventurous",
  ...
  const validateForm = () => {
    ...
    if (!formData.canTextEvery)
      errors.canTextEvery = "Text frequency is required";
    if (
      formData.age &&
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">
                Add a new AI companion to your contacts
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="text-center">
              ...
              {formData.aiAvatar && (
                <p className="mt-2 text-xs text-gray-600 truncate max-w-xs mx-auto">
                  {formData.aiAvatar.name}
                </p>
              )}
            </div>
            {/* Form Content */}
              ...
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <select
                    ...
                    className={`w-full p-4 text-base bg-white rounded-lg border-2 transition-all duration-200 min-h-[160px] ${
                ...
                <div className="grid grid-cols-2 gap-5">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        handleInputChange("canTextEvery", e.target.value)
                      }
                      className={`w-full p-4 text-base bg-white rounded-lg border-2 transition-all duration-200 ${
                  ...
                  {(formData.canTextEvery === "Weekly" ||
                    ...
                    <div>
                      ...
                      <select
                        value={formData.on}
                        onChange={(e) =>
                          handleInputChange("on", e.target.value)
                        }
                        className="w-full p-4 text-base bg-white rounded-lg border-2 border-gray-300 focus:border-blue-500 focus:ring-4 focus:ring-blue-200 transition-all duration-200 focus:outline-none"
                      >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      handleInputChange("wantToHear", e.target.value)
                    }
                    disabled={isElementsLoading}
                    className="w-full p-4 text-base bg-white rounded-lg border-2 border-gray-300 focus:border-blue-500 focus:ring-4 focus:ring-blue-200 transition-all duration-200 focus:outline-none"
                  >
...
const ElementManagementSection = ({
  elementName,
  elementValues,
  onAddValue,
  onDeleteValue,
...
const SettingsPage = () => {
  ...
  const [addAdminAiContact, { isLoading: isSubmitting }] =
    useAddAdminAiContactMutation();
  ...
  const aiContactsRaw =
    listData?.data ?? listData?.AiContacts ?? listData ?? [];
  const aiContacts = Array.isArray(aiContactsRaw.contact)
    ...
    : aiContactsRaw && aiContactsRaw._id
    ? [aiContactsRaw]
    : [];
  ...
  const handleSubmit = async (formData) => {
    ...
    try {
      const formattedAt =
        formData.canTextEvery === "Never"
          ? ""
          : toTwelveHourNoSpace(formData.at);
      ...
      const message =
        err?.data?.message ||
        err?.error ||
        "Failed to create AI Contact. Please try again.";
      setError(message);
    }
  };
  ...
  const handleAddElementValue = async (elementName, value) => {
    try {
      ...
      const message =
        err?.data?.message ||
        err?.error ||
        `Failed to add ${elementName} value. Please try again.`;
      setError(message);
  ...
  const handleDeleteElementValue = async (elementName, value) => {
    try {
      ...
      const message =
        err?.data?.message ||
        err?.error ||
        `Failed to delete ${elementName} value. Please try again.`;
      setError(message);
      setTimeout(() => setError(""), 5000);
    }
  };
  ...
  const handleEditElementValue = async (elementName, oldValue, newValue) => {
    try {
      ...
      const message =
        err?.data?.message ||
        err?.error ||
        `Failed to update ${elementName} value. Please try again.`;
      setError(message);
      setTimeout(() => setError(""), 5000);
    }
  };
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">
            Manage application elements and AI contacts
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            {manageableElements.map((element) => (
              <ElementManagementSection
                ...
                onAddValue={(value) =>
                  handleAddElementValue(element.key, value)
                }
                onDeleteValue={(value) =>
                  handleDeleteElementValue(element.key, value)
                }
                onEditValue={(oldValue, newValue) =>
                  handleEditElementValue(element.key, oldValue, newValue)
                }
